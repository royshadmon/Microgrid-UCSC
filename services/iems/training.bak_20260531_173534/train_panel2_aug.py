...
Writes services/iems/models/nilm_panel2_aug.pt
"""
...
from model_panel2 import Panel2Net as Panel3Net  # noqa: E402
from aug_oversample import make_oversampler, add_signal_noise  # noqa: E402

REPO = Path(__file__).resolve().parents[3]
NPZ = REPO / "data/panel2_windows.npz"
NORM_JSON = REPO / "services/iems/models/panel2_norm.json"
OUT_PT = REPO / "services/iems/models/nilm_panel2_aug.pt"
...
def main() -> int:
    ...
    print(f"[train-p2-aug] X_train={X_train.shape}  X_val={X_val.shape}")
    ...
    for h in HEADS:
        ...
        print(f"[train-p2-aug]   {h:16s}: train pos={n_pos} neg={n_neg} pos_weight={pos_w[h]:.2f}")
    ...
    # Build oversampler for rare-positive heads.
    # MATNilm-inspired: bias minibatches toward windows with positives for
    # heads where positive rate < 5%. Target effective per-head positive
    # rate ~= 10% after weighted sampling.
    y_train_arrays = {h: y_train[h] for h in HEADS}
    sampler, sampler_info = make_oversampler(
        y_train_arrays, HEADS,
        sparse_threshold=0.05,
        target_min_pos_rate=0.10,
    )
    print("[train-p2-aug] oversampling boosts:")
    for h, d in sampler_info.items():
        print(f"  {h:16s}: n_pos={d['n_pos']:4d}  pos_rate={d['pos_rate']:.4f}  boost={d['boost']:.2f}x")
    ...
    print(f"[train-p2-aug] Panel3Net: {n_params:,} parameters")
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        for batch in train_loader:
            X, *ys = batch
            X = add_signal_noise(X, sigma=0.02)  # MATNilm-style noise on oversampled rare-positive windows
            preds = model(X)
        ...
        print(line)
        if bad >= PATIENCE:
            print(f"[train-p2-aug] early stop @ epoch {epoch}")
            break

    if best_state is None:
        print("[train-p2-aug] FATAL: no improvement", file=sys.stderr)
        return 1
    OUT_PT.parent.mkdir(parents=True, exist_ok=True)
    torch.save(best_state, OUT_PT)
    print(f"[train-p2-aug] best epoch={best_epoch}  avg_f1={best_f1:.4f}  → {OUT_PT}")
    return 0
