...
Writes services/iems/models/nilm_panel1_aug.pt
"""
...
OUT_PT = REPO / "services/iems/models/nilm_panel1_aug.pt"
...
def main() -> int:
    ...
    print(f"[train-p1-aug] X_train={X_train.shape}  X_val={X_val.shape}")
    ...
    for h in ("hp", "sp"):
        ...
        print(f"[train-p1-aug]   {h}: train pos={n_pos} neg={n_neg} pos_weight={pos_w[h]:.2f}")
    ...
    # MATNilm-style oversample for rare-positive heads
    y_train_arrays = {"hp": y_train["hp"], "sp": y_train["sp"]}
    sampler, sampler_info = make_oversampler(
        y_train_arrays, ("hp", "sp"),
        sparse_threshold=0.05, target_min_pos_rate=0.10,
    )
    print("[train-p1-aug] oversampling boosts:")
    for h, d in sampler_info.items():
        print(f"  {h}: n_pos={d['n_pos']} pos_rate={d['pos_rate']:.4f} boost={d['boost']:.2f}x")
    ...
    print(f"[train-p1-aug] Panel1Net: {n_params:,} parameters")
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        for X, hp, sp in train_loader:
            X = add_signal_noise(X, sigma=0.02)
            p_hp, p_sp = model(X)
            loss = (
        ...
        if bad >= PATIENCE:
            print(f"[train-p1-aug] early stop @ epoch {epoch}")
            break

    if best_state is None:
        print("[train-p1-aug] FATAL: no improvement", file=sys.stderr)
        return 1
    OUT_PT.parent.mkdir(parents=True, exist_ok=True)
    torch.save(best_state, OUT_PT)
    print(f"[train-p1-aug] best epoch={best_epoch}  avg_f1={best_f1:.4f}  → {OUT_PT}")
    return 0
