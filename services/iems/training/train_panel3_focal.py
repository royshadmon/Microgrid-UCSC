...
Writes services/iems/models/nilm_panel3_focal.pt
"""
...
from losses import auto_loss  # noqa: E402

REPO = Path(__file__).resolve().parents[3]
NPZ = REPO / "data/panel3_windows.npz"
NORM_JSON = REPO / "services/iems/models/panel3_norm.json"
OUT_PT = REPO / "services/iems/models/nilm_panel3_focal.pt"
...
def main() -> int:
    ...
    print(f"[train-p3-focal] X_train={X_train.shape}  X_val={X_val.shape}")
    ...
    for h in HEADS:
        ...
        print(f"[train-p3-focal]   {h:16s}: train pos={n_pos} neg={n_neg} pos_weight={pos_w[h]:.2f}")
    ...
    print(f"[train-p3-focal] Panel3Net: {n_params:,} parameters")
    ...
    loss_fns = {}
    print("[train-p3-focal] loss assignment by head (focal_threshold=50):")
    for h in HEADS:
        fn, kind = auto_loss(pos_w[h], focal_threshold=50.0, gamma=2.0, alpha=0.25)
        loss_fns[h] = fn
        print(f"  {h:16s}: pos_weight={pos_w[h]:7.2f}  -> {kind}")
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        for batch in train_loader:
            X, *ys = batch
            preds = model(X)
            loss = sum(loss_fns[h](preds[i], ys[i]) for i, h in enumerate(HEADS))
            optim.zero_grad()
            loss.backward()
            optim.step()
        ...
        print(line)
        if bad >= PATIENCE:
            print(f"[train-p3-focal] early stop @ epoch {epoch}")
            break

    if best_state is None:
        print("[train-p3-focal] FATAL: no improvement", file=sys.stderr)
        return 1
    OUT_PT.parent.mkdir(parents=True, exist_ok=True)
    torch.save(best_state, OUT_PT)
    print(f"[train-p3-focal] best epoch={best_epoch}  avg_f1={best_f1:.4f}  → {OUT_PT}")
    return 0
