#!/usr/bin/env python3
"""Build 100-timestep windows for Panel 2 (four heads).

Reads  data/panel2_60d_labeled.parquet
Writes data/panel2_windows.npz
...
OUT_NORM = REPO / "services/iems/models/panel2_norm.json"
...
# 12 features = the prompt's 11 + panel2_w_step (mirrors Panel 1's choice
# to surface the baseline-step signal so the model sees what the rule sees).
FEATURES = [
...
def main() -> int:
    ...
    baseline = df["panel2_w"].rolling("30min", min_periods=30).min()
    df["panel2_w_step"] = (df["panel2_w"] - baseline).clip(lower=0)
    ...
    for name, d in splits.items():
        X = d["X"]
        print(f"[windows-p2] {name}: X={X.shape}")
        if X.size:
            for h in HEADS:
                y = d[f"y_{h}"]
                valid = ~np.isnan(y)
                ...
                print(f"[windows-p2]   {name}/{h:14s}: valid={n_valid}  pos={pos}  rate={rate:.4f}")

    X_train = splits["train"]["X"]
    if X_train.size == 0:
        print("[windows-p2] FATAL: train split empty", file=sys.stderr)
        return 1
    ...
    print(f"[windows-p2] wrote {OUT_NORM}")
    ...
    print(f"[windows-p2] wrote {OUT_NPZ}")
    return 0
