...
Writes services/iems/models/nilm_panel3.pt
"""
...
OUT_PT = REPO / "services/iems/models/nilm_panel3.pt"
...
def main() -> int:
    ...
    print(f"[train-p3] X_train={X_train.shape}  X_val={X_val.shape}")
    ...
    for h in HEADS:
        ...
        print(f"[train-p3]   {h:16s}: train pos={n_pos} neg={n_neg} pos_weight={pos_w[h]:.2f}")
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        print(line)
        if bad >= PATIENCE:
            print(f"[train-p3] early stop @ epoch {epoch}")
            break

    if best_state is None:
        print("[train-p3] FATAL: no improvement", file=sys.stderr)
        return 1
    OUT_PT.parent.mkdir(parents=True, exist_ok=True)
    torch.save(best_state, OUT_PT)
    print(f"[train-p3] best epoch={best_epoch}  avg_f1={best_f1:.4f}  → {OUT_PT}")
    return 0
