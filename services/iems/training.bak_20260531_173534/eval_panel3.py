#!/usr/bin/env python3
"""Evaluate Panel 3 int8 model on the held-out test split (eight heads)."""
...
ONNX = REPO / "services/iems/models/nilm_panel3.onnx"
OUT_MD = REPO / "services/iems/training/reports/panel3_eval.md"
...
def main() -> int:
    ...
    if X_test.size == 0:
        print("[eval-p3] FATAL: test split empty", file=sys.stderr)
        return 1
    ...
    OUT_MD.write_text("\n".join(lines) + "\n")
    print(f"[eval-p3] wrote {OUT_MD}")
    for h, m in head_metrics:
        print(f"  {h:16s}: F1={m['f1']:.3f}  P={m['precision']:.3f}  "
