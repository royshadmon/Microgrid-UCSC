#!/usr/bin/env python3
"""Train Panel 2 BiLSTM (four untied heads) with per-head NaN-mask BCE.

Reads  data/panel2_windows.npz
       services/iems/models/panel2_norm.json
Writes services/iems/models/nilm_panel2.pt
"""
...
OUT_PT = REPO / "services/iems/models/nilm_panel2.pt"
...
HEADS = Panel2Net.HEADS


def f1_metrics(prob: np.ndarray, y: np.ndarray, thr: float = 0.5) -> tuple[float, float, float]:
    mask = ~np.isnan(y)
    if mask.sum() == 0:
...
def main() -> int:
    ...
    print(f"[train-p2] X_train={X_train.shape}  X_val={X_val.shape}")
    ...
    for h in HEADS:
        ...
        print(f"[train-p2]   {h:14s}: train pos={n_pos} neg={n_neg} pos_weight={pos_w[h]:.2f}")
    ...
    torch.manual_seed(7)
    model = Panel2Net(in_features=X_train.shape[-1])
    n_params = sum(p.numel() for p in model.parameters())
    print(f"[train-p2] Panel2Net: {n_params:,} parameters")
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        head_str = "  ".join(
            f"{h[:3]}[F={m[h][2]:.2f} P={m[h][0]:.2f} R={m[h][1]:.2f}]"
            for h in HEADS
        )
        print(f"epoch {epoch:3d}  tloss={tloss:.4f}  vloss={vloss:.4f}  "
              f"{head_str}  avg={avg_f1:.3f}{marker}")
        if bad >= PATIENCE:
            print(f"[train-p2] early stop @ epoch {epoch}")
            break

    if best_state is None:
        print("[train-p2] FATAL: no improvement", file=sys.stderr)
        return 1
    OUT_PT.parent.mkdir(parents=True, exist_ok=True)
    torch.save(best_state, OUT_PT)
    print(f"[train-p2] best epoch={best_epoch}  avg_f1={best_f1:.4f}  → {OUT_PT}")
    return 0
